...
int id_tripulante_para_enviar=1;
int id_tripulante = 1;
int id_patota = 1;
...
void planificarSabotaje(){
	int socketSabotaje = crear_conexion(IP_I_MONGO_STORE,PUERTO_I_MONGO_STORE);
	while(true){
...
void leer_consola(){ // proximamente recibe como parm int* socket_server
	...
	while(strcmp(leido, "") != 0){
		...
		case INICIAR_PATOTA: {	//INICIAR_PATOTA 3 /home/utnso/tp-2021-1c-DuroDeAprobar/Tareas/tareasPatota1.txt 1|1 2|1
			...
			int socketPatota = crear_conexion(IP_MI_RAM_HQ,PUERTO_MI_RAM_HQ);
			...
			if(fileTarea != NULL){
				...
				int cantidadDeTripulantes= atoi(mensaje[1]);
		...
		case EXPULSAR_TRIPULANTE: {
			...
			//ENVIO MENSAJE
			int socketExpulsar = crear_conexion(IP_MI_RAM_HQ,PUERTO_MI_RAM_HQ);
...
void log_movimiento_tripulante(int id, int coordX, int coordY)
{
	char* log_msg = "El tripulante con ID %d se movió a la posición [%d,%d]";
	log_info(logger,log_msg, id, coordX, coordY);
}
void log_tripulante_cambio_de_cola_planificacion(int id, char* razon, char* cola)
{
...
void agregarTripulanteAListaReadyYAvisar(t_tripulante* tripulante){
	...
	mensaje->estado= (int) tripulante->estado;
...
int llegoATarea(t_tripulante* tripulante){
	...
	int posicionXtarea = tripulante->tareaAsignada->coordenadas->posX;
	int posicionYtarea = tripulante->tareaAsignada->coordenadas->posY;
...
//	mensajeMovimientoMongo->coordenadasDestino->posY = tripulante->coordenadas->posY;
	...
	printf("hacia la posicion: %d %d\n",mensajeMovimientoTarea->coordenadasDestino->posX,mensajeMovimientoTarea->coordenadasDestino->posY);
...
//	tripulante->socketTripulanteImongo = socketDelTripulanteConImongo;
	...
	printf("el valor del socket es: %d", socketDelTripulanteConRam);
