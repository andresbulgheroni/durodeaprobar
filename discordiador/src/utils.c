...
void serializar_variable(void* stream, void* variable, int size, int* offset) {
...
void serializar_string(void* stream, t_string* string, int* offset) {
...
void deserializar_variable(void* stream, void* variable, int size, int* offset){
...
t_string* deserializar_string(void* stream, int* offset){

	t_string* stringRespuesta = malloc(sizeof(t_string));

	deserializar_variable(stream, &(stringRespuesta->length), sizeof(int), offset);
...
t_paquete* recibir_paquete(int32_t socket){
	...
	if(leer_buffer(paquete->codigo)){
		...
		status = recv(socket, &(paquete->buffer->size), sizeof(int), MSG_WAITALL);
...
t_buffer* serializar_iniciar_patota_msg(iniciar_patota_msg* mensaje){
	...
	buffer->size = sizeof(mensaje->idPatota) + sizeof(mensaje->tareas->length) + sizeof(mensaje->cant_tripulantes)
			+ list_size(mensaje->tripulantes) * (sizeof(int) * 3)
	...
	serializar_variable(buffer->stream, &(mensaje->idPatota), sizeof(int), &offset);
...
void liberar_conexion(int socketCliente){
